...
class BaseMode(GameMode,ABC):
    ...
    def execute_round(self,player_position):
        ...
        if player_role == Role.hider:
            seeker_score = self.payoff_matrix[player_position][computer_position] * -1
            hider_score = self._proximity_score(self.payoff_matrix[player_position][computer_position], player_position,
            ...
            hider_score = self._proximity_score(self.payoff_matrix[computer_position][player_position], player_position,
                                                computer_position, world_dimension)
            self.game_state.score_tracker.update_score(hider_score, seeker_score, player_role)
